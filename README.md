...
- 1h/24h live flows, unique wallets, net flow and a 24-hour chart; longer windows remain coverage-gated until enough local history exists;
...
- restart-safe local event cache;
...
Solana JSON-RPC
  ...
  └─ Program signature/transaction poll (8s)
              ↓
       zero-dependency decoder
              ↓
  data/events.json + in-memory state
              ↓
...
| `GET /api/stats` | 1h/24h/7d aggregates and chart buckets |
...
- `withdraw` has no amount argument; amount is derived from the successful transaction's vault token-balance delta.
...
- 24h/7d history covers only locally indexed events. The UI explicitly shows its coverage start.
- The public Solana endpoint rate-limits transaction backfill. Defaults deliberately serialize calls at four requests/second and start with 25 events.
...
3. PostgreSQL before deep historical backfill;
