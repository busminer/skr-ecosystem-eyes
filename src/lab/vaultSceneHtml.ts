...
function syncQueue(items){ var keys=items.map(function(q){return q.k;}).join(','); if(keys===S.lastQueueKeys){ items.forEach(function(q){ var h=S.hang.find(function(x){return x.key===q.k;}); if(h){ h.amt=q.amount; h.unlockAt=q.unlockAt; } }); return; } S.lastQueueKeys=keys;
  var keep={}; var slots=Math.max(1,items.length); items.forEach(function(q,i){ keep[q.k]=1; var h=S.hang.find(function(x){return x.key===q.k;}); if(!h){ var n=Math.max(1,Math.min(6,Math.round(Math.log10(Math.max(1000,q.amount)/1e4))+2)); var raft=[]; for(var j=0;j<n;j++) raft.push({dx:rnd(-11,11),dy:rnd(-4,4),L:q.amount>=1e5?Math.floor(rnd(1,3)):1,rot:rnd(-0.2,0.2)}); var x=slots===1?W/2:34+(W-68)*(i/(slots-1)); S.hang.push({key:q.k,x:x,y:H*0.45,vy:-14,hangY:(i%2===0?H*0.27:H*0.39),t:0,hold:1e9,amt:q.amount,who:q.who,unlockAt:q.unlockAt,startAt:q.startAt,raft:raft,big:q.amount>=1e5,state:'rise',life:1,sw:Math.random()*6}); } });
...
function draw(){ var c=ctx; c.clearRect(0,0,W,H);
  ...
  c.fillStyle='rgba(255,196,107,.035)'; c.fillRect(0,H*0.2,W,H*0.26); c.strokeStyle='rgba(255,196,107,.1)'; c.setLineDash([2,6]); c.lineWidth=1; c.beginPath(); c.moveTo(0,H*0.2); c.lineTo(W,H*0.2); c.moveTo(0,H*0.46); c.lineTo(W,H*0.46); c.stroke(); c.setLineDash([]);
  c.fillStyle='rgba(255,196,107,.5)'; c.font='600 8.5px sans-serif'; c.textAlign='left'; c.fillText('COOLING 48H · '+fmt(S.pending||0)+' SKR',12,H*0.2-6);
  ...
  c.restore();
  if(S.me){ var mx=S.meX,my=pileTop(mx)+10; drawGlow(c,GD,mx,my,26,0.75); for(var k=0;k<5;k++) drawPhone(1,true,'gold',k===2?'you':'',mx-14+k*7,my+(k%2)*3,(k-2)*0.03,1); c.fillStyle='#F2E7C9'; c.font='600 10px sans-serif'; c.textAlign='center'; c.fillText(S.me.name+' · '+fmt(S.me.amount)+(S.me.days?' · '+S.me.days+' days':''),mx,my-16); }
  ...
  S.hang.forEach(function(h){ var a=Math.min(1,h.life); var tone=h.state==='ready'||h.state==='fly'?'in':'out'; var col=tone==='in'?IN:OUT; drawGlow(c,col,h.x,h.y,h.big?30:16,0.38*a); h.raft.forEach(function(r){ drawPhone(r.L,true,tone,'',h.x+r.dx,h.y+r.dy,r.rot+Math.sin(S.t+h.sw)*0.05,a); });
    if(h.state==='hang'||h.state==='ready'){ var R=(h.big?26:14); var frac=h.unlockAt&&h.startAt?Math.max(0,Math.min(1,1-(S.now-h.startAt)/172800)):Math.max(0,1-h.t/h.hold); c.strokeStyle=h.state==='ready'?'rgba(124,240,188,.9)':'rgba(255,196,107,.85)'; c.lineWidth=1.3; c.globalAlpha=a; c.beginPath(); c.arc(h.x,h.y,R,-1.571,-1.571+6.283*frac); c.stroke(); c.strokeStyle='rgba(255,255,255,.08)'; c.beginPath(); c.arc(h.x,h.y,R,0,6.283); c.stroke();
      c.fillStyle=h.state==='ready'?'#9ff6d2':'#ffd58f'; c.font='700 10px sans-serif'; c.textAlign='center'; c.fillText(fmt(h.amt)+' SKR',h.x,h.y+R+12); c.fillStyle='rgba(255,255,255,.6)'; c.font='500 8px sans-serif'; c.fillText(h.state==='ready'?'ready':h.unlockAt?left(h.unlockAt-S.now):'asked out',h.x,h.y+R+22); if(h.who&&h.big){ c.fillStyle='rgba(201,169,106,.9)'; c.fillText(h.who,h.x,h.y+R+32); } c.globalAlpha=1; } });
  ...
  c.fillStyle='rgba(84,103,122,.9)'; c.font='500 8px sans-serif'; c.textAlign='left'; c.fillText('drizzle is density, not transactions · labels are finalized moves',12,H-6);
