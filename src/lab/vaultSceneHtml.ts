...
function draw(){ var c=ctx; c.clearRect(0,0,W,H); var Z=S.zoom.k, K=S.storyK, LBL=Z<1.3;
  ...
  if(LBL){ c.fillStyle='rgba(255,196,107,.5)'; c.font='600 8.5px sans-serif'; c.textAlign='left'; c.fillText('COOLING 48H · '+fmt(S.pending||0)+' SKR',12,H*0.2-6); }
  c.beginPath(); c.moveTo(0,H); for(var i=0;i<S.cols;i++){ var x=i*8; var y=H-(S.pile[i]+S.bump[i])*K+(S.shock>0?Math.sin(i*0.7+S.t*14)*3*S.shock:0); c.lineTo(x,y);} c.lineTo(W,H); c.closePath(); var pg=c.createLinearGradient(0,H*0.5,0,H); pg.addColorStop(0,'#0F2A36'); pg.addColorStop(1,'#050C12'); c.fillStyle=pg; c.fill();
  ...
  c.restore();
  if(S.me){ var mx=S.meX,my=pileTop(mx)+10; if(!LBL){ c.fillStyle='#F2E7C9'; c.font='600 3.4px sans-serif'; c.textAlign='center'; c.fillText(S.me.name+' · '+fmt(S.me.amount)+' SKR · this is you',mx,my-9); } drawGlow(c,GD,mx,my,26,0.75); for(var k=0;k<5;k++) drawPhone(1,true,'gold',k===2?'you':'',mx-14+k*7,my+(k%2)*3,(k-2)*0.03,1); if(LBL){ c.fillStyle='#F2E7C9'; c.font='600 10px sans-serif'; c.textAlign='center'; c.fillText(S.me.name+' · '+fmt(S.me.amount)+(S.me.days?' · '+S.me.days+' days':''),mx,my-16); } }
  if(LBL&&S.last&&S.t-S.last.at>40){ c.fillStyle='rgba(255,196,107,.45)'; c.font='600 9px sans-serif'; c.textAlign='center'; c.fillText('LAST BIG MOVE · '+fmt(S.last.v)+' '+S.last.kind.toUpperCase()+(S.last.who?' · '+S.last.who:''),W/2,H*0.475); }
  S.hang.forEach(function(h){ var a=Math.min(1,h.life); var tone=h.state==='ready'||h.state==='fly'?'ready':'out'; var col=tone==='ready'?CY:OUT; drawGlow(c,col,h.x,h.y,h.big?30:16,0.38*a); h.raft.forEach(function(r){ drawPhone(r.L,true,tone,'',h.x+r.dx,h.y+r.dy,r.rot+Math.sin(S.t+h.sw)*0.05,a); });
