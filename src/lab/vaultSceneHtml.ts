...
function sprite(L,lit,tone,text){ var k=L+'|'+lit+'|'+tone+'|'+(text||''); if(spriteCache[k]) return spriteCache[k]; var w=SZ[L][0],h=SZ[L][1]; var c=document.createElement('canvas'); c.width=Math.ceil((w+8)*DPR*2); c.height=Math.ceil((h+8)*DPR*2); var x=c.getContext('2d'); x.scale(DPR*2,DPR*2); x.translate(4,4);
  var body=tone==='out'?'#5a2f26':tone==='gold'?'#6b5630':'#2a3f4c', edge=tone==='out'?'#ff9f7a':tone==='gold'?'#e8cf95':'#5f8494';
  x.fillStyle=body; x.strokeStyle=edge; x.lineWidth=0.7; x.beginPath(); x.roundRect(0,0,w,h,w*0.22); x.fill(); x.stroke();
  var scr=lit?(tone==='out'?'#ffb08a':tone==='gold'?'#ffe9b8':'#9ff6d2'):'#173a48';
...
var S={t:0,frozen:false,motion:'live',paused:false,idle:0,p:[],hang:[],tags:[],cols:0,pile:[],bump:[],piled:[],shock:0,percent:0.47,todayIn:0,todayOut:0,dustGap:0.4,me:null,last:null,now:Math.floor(Date.now()/1000),tickAt:performance.now(),ready:false,lastQueueKeys:''};
function pileTop(x){ var i=Math.min(S.cols-1,Math.max(0,Math.floor(x/8))); return H-S.pile[i]-S.bump[i]; }
...
function initPile(){ S.cols=Math.floor(W/8)+1; S.pile=[]; S.bump=[]; for(var i=0;i<S.cols;i++){ S.pile.push(H*S.percent*(0.97+0.04*Math.sin(i*0.22)+0.015*Math.sin(i*0.9))); S.bump.push(0);} S.piled=[]; for(var j=0;j<Math.round(S.cols*2.4);j++){ var col=Math.floor(Math.random()*S.cols); var depth=Math.pow(Math.random(),2)*0.4; S.piled.push({x:col*8+rnd(1,6),y:H-(1-depth)*(S.pile[col]-6),L:Math.floor(rnd(1,3)),rot:rnd(-0.12,0.12),dim:0.45+0.5*(1-depth/0.4),lit:0,tone:'in'}); } S.piled.sort(function(a,b){return a.y-b.y;}); S.meX=W*0.68; }
function retarget(){ // the pile follows the real share smoothly
  for(var i=0;i<S.cols;i++){ var target=H*S.percent*(0.97+0.04*Math.sin(i*0.22)+0.015*Math.sin(i*0.9)); S.pile[i]+=(target-S.pile[i])*0.02; } }
...
function spawnStake(amount,who,sig,x){ var big=amount>=1e5; x=x!=null?x:rnd(30,W-30);
  if(!big){ S.p.push({k:'in',x:x,y:-18,vy:52,vx:0,L:2,lit:true,tone:'in',sw:Math.random()*6,life:1,amt:amount,who:who,sig:sig,text:fmt(amount)}); return; }
  ...
  tag(x,H*0.5,fmt(amount)+' SKR staked',who,'#9ff6d2',2.6,true); S.last={v:amount,who:who,kind:'staked',at:S.t}; }
function liftExit(amount,who,sig,big){ var col=Math.floor(rnd(4,S.cols-4)); var x=col*8+4; var top=pileTop(x); for(var i=0;i<S.cols;i++){ var d=Math.abs(i-col); if(d<5) S.bump[i]+=6*(1-d/5); }
  ...
  S.hang.push({x:x,y:top-6,vy:-rnd(16,22),hangY:rnd(H*0.23,H*0.40),t:0,hold:big?9:6,amt:amount,who:who,sig:sig,raft:raft,big:big,state:'rise',life:1,sw:Math.random()*6,temp:true,frac:0});
  if(big){ tag(x,H*0.46,fmt(amount)+' SKR asked out',who,'#ffb08a',2.6,true); S.last={v:amount,who:who,kind:'asked out',at:S.t}; } }
function tag(x,y,txt,sub,tone,hold,rise,big){ if(S.tags.length>1){ S.tags.shift(); } S.tags.push({x:Math.max(70,Math.min(W-70,x)),y:y,txt:txt,sub:sub,tone:tone,t:0,hold:hold,rise:rise,big:big}); }
...
function syncQueue(items){ var keys=items.map(function(q){return q.k;}).join(','); if(keys===S.lastQueueKeys){ items.forEach(function(q){ var h=S.hang.find(function(x){return x.key===q.k;}); if(h){ h.amt=q.amount; h.unlockAt=q.unlockAt; } }); return; } S.lastQueueKeys=keys;
  var keep={}; items.forEach(function(q,i){ keep[q.k]=1; var h=S.hang.find(function(x){return x.key===q.k;}); if(!h){ var n=Math.max(1,Math.min(12,Math.round(Math.log10(Math.max(1000,q.amount)/1e3))+1)); var raft=[]; for(var j=0;j<n;j++) raft.push({dx:rnd(-14,14),dy:rnd(-5,5),L:q.amount>=1e5?Math.floor(rnd(1,3)):2,rot:rnd(-0.2,0.2)}); var x=30+((i*0.618)%1)*(W-60); S.hang.push({key:q.k,x:x,y:H*0.45,vy:-14,hangY:H*0.22+((i*0.382)%1)*H*0.2,t:0,hold:1e9,amt:q.amount,who:q.who,unlockAt:q.unlockAt,startAt:q.startAt,raft:raft,big:q.amount>=1e5,state:'rise',life:1,sw:Math.random()*6}); } });
...
function draw(){ var c=ctx; c.clearRect(0,0,W,H);
  var mood=S.todayOut>S.todayIn?0:1; var g=c.createLinearGradient(0,0,0,H); g.addColorStop(0,'#04070B'); g.addColorStop(0.55,mood?'#0A1C2A':'#08141C'); g.addColorStop(1,'#0B1F2B'); c.fillStyle=g; c.fillRect(0,0,W,H);
  c.fillStyle='rgba(255,196,107,.04)'; c.fillRect(0,H*0.2,W,H*0.24); c.strokeStyle='rgba(255,196,107,.14)'; c.setLineDash([2,6]); c.lineWidth=1; c.beginPath(); c.moveTo(0,H*0.2); c.lineTo(W,H*0.2); c.moveTo(0,H*0.44); c.lineTo(W,H*0.44); c.stroke(); c.setLineDash([]);
  c.fillStyle='rgba(255,196,107,.6)'; c.font='600 9px sans-serif'; c.textAlign='left'; c.fillText('COOLING · 48H · '+fmt(S.pending||0)+' SKR',12,H*0.2-6);
  c.fillStyle='rgba(124,240,188,.55)'; c.textAlign='right'; c.fillText('READY LEAVES HERE ↑',W-12,H*0.14);
  c.beginPath(); c.moveTo(0,H); for(var i=0;i<S.cols;i++){ var x=i*8; var y=H-S.pile[i]-S.bump[i]+(S.shock>0?Math.sin(i*0.7+S.t*14)*3*S.shock:0); c.lineTo(x,y);} c.lineTo(W,H); c.closePath(); var pg=c.createLinearGradient(0,H*0.5,0,H); pg.addColorStop(0,'#0F2A36'); pg.addColorStop(1,'#050C12'); c.fillStyle=pg; c.fill();
  var pe=c.createLinearGradient(0,H*0.5,0,H*0.63); pe.addColorStop(0,'rgba(124,240,188,.2)'); pe.addColorStop(1,'rgba(124,240,188,0)'); c.fillStyle=pe; c.fill();
  c.save(); c.clip();
  S.piled.forEach(function(q){ if(q.lit>0.05){ drawGlow(c,q.tone==='gold'?GD:IN,q.x,q.y,13,q.lit*0.8); } drawPhone(q.L,q.lit>0.05,q.tone,q.text,q.x,q.y,q.rot,q.lit>0.05?1:q.dim); });
  c.restore();
  c.fillStyle='rgba(124,240,188,.6)'; c.font='600 9px sans-serif'; c.textAlign='right'; c.fillText((S.percent*100).toFixed(2)+'% STAKED',W-12,pileTop(W-30)-8);
  if(S.me){ var mx=S.meX,my=pileTop(mx)+8; drawGlow(c,GD,mx,my,26,0.75); for(var k=0;k<5;k++) drawPhone(1,true,'gold',k===2?'you':'',mx-14+k*7,my+(k%2)*3,(k-2)*0.03,1); c.fillStyle='#F2E7C9'; c.font='600 10px sans-serif'; c.textAlign='center'; c.fillText(S.me.name+' · '+fmt(S.me.amount)+(S.me.days?' · '+S.me.days+' days':''),mx,my-16); }
  if(S.last&&S.t-S.last.at>40){ c.fillStyle='rgba(255,196,107,.45)'; c.font='600 9px sans-serif'; c.textAlign='center'; c.fillText('LAST BIG MOVE · '+fmt(S.last.v)+' '+S.last.kind.toUpperCase()+(S.last.who?' · '+S.last.who:''),W/2,H*0.475); }
  S.hang.forEach(function(h){ var a=Math.min(1,h.life); var tone=h.state==='ready'||h.state==='fly'?'in':'out'; var col=tone==='in'?IN:OUT; drawGlow(c,col,h.x,h.y,h.big?34:18,0.6*a); h.raft.forEach(function(r){ drawPhone(r.L,true,tone,'',h.x+r.dx,h.y+r.dy,r.rot+Math.sin(S.t+h.sw)*0.05,a); });
    if(h.state==='hang'||h.state==='ready'){ var R=(h.big?26:14); var frac=h.unlockAt&&h.startAt?Math.max(0,Math.min(1,1-(S.now-h.startAt)/172800)):Math.max(0,1-h.t/h.hold); c.strokeStyle=h.state==='ready'?'rgba(124,240,188,.9)':'rgba(255,196,107,.85)'; c.lineWidth=1.3; c.globalAlpha=a; c.beginPath(); c.arc(h.x,h.y,R,-1.571,-1.571+6.283*frac); c.stroke(); c.strokeStyle='rgba(255,255,255,.08)'; c.beginPath(); c.arc(h.x,h.y,R,0,6.283); c.stroke();
      c.fillStyle=h.state==='ready'?'#9ff6d2':'#ffd58f'; c.font='700 10px sans-serif'; c.textAlign='center'; c.fillText(fmt(h.amt)+' SKR',h.x,h.y+R+12); c.fillStyle='rgba(255,255,255,.6)'; c.font='500 8.5px sans-serif'; c.fillText((h.state==='ready'?'ready to withdraw':h.unlockAt?left(h.unlockAt-S.now):'asked out')+(h.who?' · '+h.who:''),h.x,h.y+R+23); c.globalAlpha=1; } });
  [0,1,2].forEach(function(L){ S.p.forEach(function(q){ if(q.L!==L) return; var rot=q.gold?0:Math.sin(S.t*2+q.sw)*0.15; var al=q.life*(0.5+L*0.25); if(q.lit){ drawGlow(c,q.tone==='gold'?GD:IN,q.x,q.y,SZ[L][1]*1.3,0.7); al=1; } drawPhone(L,q.lit,q.tone,q.text,q.x,q.y,rot,al); if(q.lit&&q.text&&L===2&&!q.gold){ c.fillStyle='rgba(159,246,210,.9)'; c.font='700 9px sans-serif'; c.textAlign='left'; c.fillText(q.text,q.x+9,q.y+3); } }); });
  ...
  c.fillStyle='rgba(84,103,122,.9)'; c.font='500 8px sans-serif'; c.textAlign='left'; c.fillText('drizzle is density, not transactions · every label is a finalized move',12,H-8);
...
function loop(now){ var dt=Math.min(0.05,(now-last)/1000); last=now; a2+=dt; var calm=S.motion==='calm'||(S.motion==='live'&&S.idle>90); var frame=calm?1:1/30; if(a2>=frame&&!S.paused&&S.motion!=='off'){ var st=Math.min(a2,calm?1:0.1); a2=0; if(now-S.tickAt>1000){ S.now=Math.floor(Date.now()/1000); S.tickAt=now; } if(!S.frozen) step(st); draw(); } requestAnimationFrame(loop); }
size(); for(var i=0;i<30;i++){ spawnDust(); S.p[S.p.length-1].y=rnd(0,H*0.5); } draw(); requestAnimationFrame(loop);
window.addEventListener('resize',size);
document.addEventListener('touchstart',function(){ S.idle=0; },{passive:true});
cv.addEventListener('click',function(e){ var x=e.clientX, y=e.clientY, hit=null; S.hang.forEach(function(h){ if(Math.hypot(h.x-x,h.y-y)<32) hit={kind:'exit',amount:h.amt,who:h.who||'',sig:h.sig||'',unlockAt:h.unlockAt||0}; }); S.p.forEach(function(q){ if(q.lit&&q.text&&!q.gold&&Math.hypot(q.x-x,q.y-y)<18) hit={kind:'stake',amount:q.amt||0,who:q.who||'',sig:q.sig||''}; }); if(hit&&window.ReactNativeWebView){ window.ReactNativeWebView.postMessage(JSON.stringify({type:'tap',hit:hit})); } S.idle=0; });
window.__push=function(m){ try{ if(!m) return;
  if(m.type==='state'){ if(typeof m.percent==='number') S.percent=Math.max(0.02,Math.min(0.95,m.percent/100)); S.pending=m.pending||0; if(typeof m.todayIn==='number') S.todayIn=m.todayIn; if(typeof m.todayOut==='number') S.todayOut=m.todayOut; if(m.eventsLastHour){ S.dustGap=Math.max(0.15,Math.min(2.5,3600/Math.max(20,m.eventsLastHour)/2)); } if(m.queue) syncQueue(m.queue); if(m.now) S.now=m.now; }
  else if(m.type==='events'){ (m.items||[]).forEach(function(e){ var a=e.amount||0; if(e.kind==='stake'){ if(a>=1000) spawnStake(a,e.who,e.sig); else { spawnDust(); } } else if(e.kind==='unstake'){ if(a>=1000) liftExit(a,e.who,e.sig,a>=1e5); } else if(e.kind==='withdraw'){ var h=S.hang.find(function(x){return x.who&&x.who===e.who&&x.state!=='fly';}); if(h){ h.state='fly'; h.vy=0; } if(a>=1000) tag(rnd(60,W-60),H*0.3,fmt(a)+' SKR withdrawn',e.who,'#9ff6d2',2,true); } else if(e.kind==='cancel_unstake'){ var h2=S.hang.find(function(x){return x.who&&x.who===e.who&&x.state==='hang';}); if(h2){ h2.state='fall'; h2.vy=10; } tag(rnd(60,W-60),H*0.34,'exit cancelled'+(a>=1000?' · '+fmt(a)+' SKR':''),e.who,'#56E0FF',1.8,false); } }); }
