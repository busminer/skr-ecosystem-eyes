...
function fmt(n){ n=+n||0; if(n>=1e9) return (n/1e9).toFixed(2)+'B'; if(n>=1e6) return (n/1e6).toFixed(n>=1e7?0:1)+'M'; if(n>=1e3) return (n/1e3).toFixed(n>=1e4?0:1)+'K'; return String(Math.round(n)); }
...
function size(){ if(window.innerWidth<2||window.innerHeight<2){ setTimeout(size,120); return; } W=window.innerWidth; H=window.innerHeight; cv.width=Math.round(W*DPR); cv.height=Math.round(H*DPR); ctx.setTransform(DPR,0,0,DPR,0,0); initPile(); }
function pileH(){ return H*(0.17+S.percent*0.26); }
function initPile(){ S.cols=Math.floor(W/8)+1; S.pile=[]; S.bump=[]; for(var i=0;i<S.cols;i++){ S.pile.push(pileH()*(0.97+0.04*Math.sin(i*0.22)+0.015*Math.sin(i*0.9))); S.bump.push(0);} S.piled=[]; for(var j=0;j<Math.round(S.cols*2.4);j++){ var col=Math.floor(Math.random()*S.cols); var depth=Math.pow(Math.random(),1.3)*0.86; S.piled.push({x:col*8+rnd(1,6),y:H-(1-depth)*(S.pile[col]-6),L:depth>0.55?0:Math.floor(rnd(0,2)),rot:rnd(-0.12,0.12),dim:0.2+0.5*(1-depth/0.86),lit:0,tone:'in',win:Math.random()<S.lit,wph:Math.random()*6}); } S.piled.sort(function(a,b){return a.y-b.y;}); S.meX=W*0.8; }
...
function background(c){ var mood=S.todayOut>S.todayIn?0:1; var key=W+'x'+H+'|'+mood+'|'+(S.night?1:0); if(bgKey!==key){ BG=document.createElement('canvas'); BG.width=Math.round(W*DPR); BG.height=Math.round(H*DPR); var x=BG.getContext('2d'); x.scale(DPR,DPR); var g=x.createLinearGradient(0,0,0,H); g.addColorStop(0,S.night?'#070E15':'#0A1722'); g.addColorStop(0.5,S.night?'#081420':mood?'#0C2030':'#0A1A26'); g.addColorStop(1,S.night?'#0A1A24':'#0B1F2B'); x.fillStyle=g; x.fillRect(0,0,W,H);
  ...
  bgKey=key; } if(BG.width>0&&BG.height>0) c.drawImage(BG,0,0,W,H); }
function draw(){ var c=ctx; var Z=S.zoom.k, K=S.storyK, LBL=Z<1.3;
  ...
  c.beginPath(); c.moveTo(0,H); for(var i=0;i<S.cols;i++){ var x=i*8; var y=H-(S.pile[i]+S.bump[i])*K+(S.shock>0?Math.sin(i*0.7+S.t*14)*3*S.shock:0); c.lineTo(x,y);} c.lineTo(W,H); c.closePath(); var ptop=Math.round(H-pileH()*K); if(!S.pg||S.pgH!==ptop){ S.pg=c.createLinearGradient(0,ptop-10,0,H); S.pg.addColorStop(0,'#123A48'); S.pg.addColorStop(0.35,'#0D2733'); S.pg.addColorStop(1,'#040A10'); S.pe=c.createLinearGradient(0,ptop-4,0,ptop+H*0.12); S.pe.addColorStop(0,'rgba(124,240,188,.22)'); S.pe.addColorStop(1,'rgba(124,240,188,0)'); S.pgH=ptop; } c.fillStyle=S.pg; c.fill(); c.fillStyle=S.pe; c.fill();
  c.save(); c.clip();
  // strata through the bedrock, drawn inside the pile's clip so the clip path itself stays untouched
  c.strokeStyle='rgba(124,240,188,.075)'; c.lineWidth=1; [0.3,0.55,0.78].forEach(function(f){ c.beginPath(); for(var i=0;i<S.cols;i++){ var sx=i*8, sy=H-(S.pile[i]+S.bump[i])*K; c.lineTo(sx,sy+(H-sy)*f+Math.sin(i*0.35+f*9)*1.5); } c.stroke(); });
  ...
  S.hang.forEach(function(h){ var a=Math.min(1,h.life); var tone='out'; var col=OUT; drawGlow(c,col,h.x,h.y,h.big?30:16,0.38*a); h.raft.forEach(function(r){ drawPhone(r.L,true,tone,'',h.x+r.dx,h.y+r.dy,r.rot+Math.sin(S.t*0.6+h.sw)*0.03,a); });
  ...
  if(LBL&&S.held){ var ht='HELD · '+fmt(S.held)+' SKR'+(S.positions?' · '+S.positions.toLocaleString('en-US')+' positions':''); c.font='600 10px sans-serif'; c.textAlign='left'; var hw=c.measureText(ht).width; c.fillStyle='rgba(4,10,16,.55)'; c.beginPath(); c.roundRect(6,H-22,hw+12,17,5); c.fill(); c.fillStyle='rgba(159,246,210,.85)'; c.fillText(ht,12,H-9); }
  if(S.night&&LBL){ c.fillStyle='rgba(255,228,170,.6)'; c.font='600 8.5px sans-serif'; c.textAlign='right'; c.fillText('NIGHT · lit windows staked today',W-12,H*0.09-5); }
